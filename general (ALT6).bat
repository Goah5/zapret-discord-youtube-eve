--new ^
--filter-l3=ipv4 --filter-tcp=5222,443,26000,%GameFilter% --dpi-desync=syndata --ipset="%LISTS%ipset-eve.txt" --new ^
--filter-l3=ipv4 --filter-tcp=5222,443,26000,%GameFilter% --dpi-desync=syndata --hostlist="%LISTS%list-eve.txt"
@echo off
chcp 65001 > nul
:: 65001 - UTF-8
...
start "zapret: %~n0" /min "%BIN%winws.exe" --wf-tcp=80,443,5222,26000,%GameFilter% --wf-udp=443,50000-50100,%GameFilter% ^
...
--filter-udp=%GameFilter% --ipset="%LISTS%ipset-all.txt" --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=12 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="%BIN%quic_initial_www_google_com.bin" --dpi-desync-cutoff=n2--new ^
--filter-l3=ipv4 --filter-tcp=5222,443,26000,%GameFilter% --dpi-desync=syndata --ipset="%LISTS%ipset-eve.txt" --new ^
--filter-l3=ipv4 --filter-tcp=5222,443,26000,%GameFilter% --dpi-desync=syndata --hostlist="%LISTS%list-eve.txt"
